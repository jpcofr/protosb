...
int main(int argc, char const *argv[])
{
  ...
  server_address.sin_addr.s_addr = INADDR_ANY; // (struct in_addr) Internet address, a plain integer. (u_int32_t)0x00000000
  ...
  struct epoll_event server_epoll_config, epoll_events[MAX_EVENTS];
  ...
  if (
      ...
      epoll_ctl(epoll_file_descriptor,  // epoll file descriptor.
                ...
                server_file_descriptor, // Server's socket/file descriptor.
                &server_epoll_config    // See the explanation some lines before this one.
                ) == -1)
  ...
  for (;;)
  {
    ready_file_descriptors = // Up to maxevents are returned. The argument must be greater than zero.
        // This system call waits for events on the epoll(7) instance
        // referred to by the file descriptor. (See https://bit.ly/2CosMEK)
        epoll_wait(epoll_file_descriptor, // epoll file descriptor.
                   epoll_events,          //
                   MAX_EVENTS,            //
    ...
    for (int n = 0; n < ready_file_descriptors; ++n)
    {
      if (epoll_events[n].data.fd == server_file_descriptor)
      {
        incoming_connection_socket = accept(server_file_descriptor, (struct sockaddr *)&server_address, (socklen_t *)&server_address_length);
        if (incoming_connection_socket == -1)
          server_error("accept");

        int flags_for_fcntl = 1;
        fcntl(incoming_connection_socket, O_NONBLOCK, &flags_for_fcntl); // the socket must not block the program.
        server_epoll_config.events = EPOLLIN | EPOLLET;                  // EPOLLET: Sets the Edge Triggered behavior (See https://bit.ly/2Cpwqy1)
        server_epoll_config.data.fd = incoming_connection_socket;
        if (epoll_ctl(epoll_file_descriptor, EPOLL_CTL_ADD, incoming_connection_socket, &server_epoll_config) == -1)
      ...
      else
      {
        ...
        if (read(epoll_events[n].data.fd, buf, 1024) < 0)
          server_error("read");
